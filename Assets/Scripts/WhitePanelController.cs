...
public class WhitePanelController : MonoBehaviour {
	...
	void setColors() {
		for(int i=0; i<choices.Length; i++) {
			GameObject go = GameObject.Find("VerticalPanelWL"+(i+1));//.GetComponent<Transform>
	...
	void setGColors() {
		for(int i=0; i<choices.Length; i++) {
			GameObject go = GameObject.Find("VerticalPanelWG"+(i+1));//.GetComponent<Transform>
			go.transform.Find("BlackSphere").transform.renderer.enabled = false;
			if(i==chosenPanel)
				go.transform.Find("BlackSphere").transform.renderer.enabled = true;
			go.transform.Find("WhiteFront").transform.renderer.material = (Material) Resources.Load("Grey", typeof(Material));
	...
	void checkVictory(){
		GameObject.Find("CubeGlobal").GetComponent<CubeController>().choices[3] = choices;
		if(choices.SequenceEqual(shouldBe)){
			canUse=false;
			
			GameObject.Find("CubeGlobal").GetComponent<CubeController>().minigamesState[3]= 1;
			GameObject.Find("CubeGlobal").GetComponent<CubeController>().checkIfGameWasWon();
			GameObject.Find("White/PanelWallContainer/Panel").SetActiveRecursively(false);

			GameObject.Find("VerticalPanelWL1").SetActive(false);
			GameObject.Find("VerticalPanelWL2").SetActive(false);
			GameObject.Find("VerticalPanelWL3").SetActive(false);
			GameObject.Find("VerticalPanelWL4").SetActive(false);
			GameObject.Find("VerticalPanelWL5").SetActive(false);
